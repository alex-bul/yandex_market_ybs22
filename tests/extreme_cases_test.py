...
def test_sales_intervals():
    ...
    try:
        ...
        sales_ok = {'items': [
            {'id': 'd4351384-f3ef-11ec-b939-0242ac120002', 'name': 'Машина', 'parentId': None, 'type': 'OFFER',
             'price': 10,
             'date': '2022-05-01T12:00:00.000Z'}]}

        sales_ok['items'].sort(key=lambda x: x['id'])
        ...
        result_data['items'].sort(key=lambda x: x['id'])
        assert result_data == sales_ok
...
def test_statistic_intervals():
    ...
    try:
        ...
        assert response.json() == {'items': [
            {'id': '002fe7de-f3f5-11ec-b939-0242ac120002', 'name': 'Машина', 'parentId': None, 'type': 'OFFER',
             'price': 10,
             'date': '2022-05-01T12:00:00.000Z'}]}
        ...
        result_data = response.json()
        result_data['items'].sort(key=lambda x: x['id'])
        excepted['items'].sort(key=lambda x: x['id'])

        assert result_data == excepted
